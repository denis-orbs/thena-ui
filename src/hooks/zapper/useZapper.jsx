...
export const useZapperAddLiquidity = () => {
  ...
  const { startTxn, writeTxn, endTxn, sendTxn, updateTxn } = useTxn()
  ...
  const handleAddLiquidity = useCallback(
    async ({ token, amount, mintInfo, route, deadline = 1800000000, isFarming = false }) => {
      try {
        ...
        if (!isApproved) {
          ...
          updateTxn({ key, approveId, status: TXN_STATUS.SUCCESS, hash: '' })
        }
    ...
    [account, t, startTxn, chainId, sendTxn, endTxn, writeTxn, updateTxn],
  )
