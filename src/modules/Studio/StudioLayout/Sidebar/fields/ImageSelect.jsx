...
function ImageSelect({ options, selectedOption, setSelectedOption }) {
  ...
  return (
    <div className='relative lg:w-65'>
      {/* Select Button */}
      {isLgDown ? (
        <TextButton className='size-11' onClick={() => setIsOpen(!isOpen)}>
        ...
        <EmphasisButton
          ...
          className='flex w-full items-center justify-between rounded-lg bg-neutral-700 p-3 font-normal text-neutral-50 backdrop-blur-sm transition-all duration-200 hover:bg-gray-700/50'
        >
          ...
          {(options || []).map(option => (
            <div
              ...
              className={cn(
                ...
                option.id === selectedOption?.id && 'bg-neutral-950/50',
              )}
            >
              <Image
                alt={option.name}
