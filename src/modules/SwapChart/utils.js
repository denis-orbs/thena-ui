...
export const NUMBER_CHART_DATA = 1499
export const NUMBER_VISIBLE_CHART_DATA = 30
...
export const getClosePriceChange = lineChartData => {
  if (lineChartData.length > 0) {
    const firstValue = lineChartData.find(({ close }) => !!close && close > 0)?.close ?? 0
    const lastValue = lineChartData[lineChartData.length - 1].close
    const changeValue = lastValue - firstValue
