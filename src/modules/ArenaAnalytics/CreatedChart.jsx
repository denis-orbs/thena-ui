...
const V4_TC_CREATED_ANALYTICS = gql`
  query V4_TC_CREATED_ANALYTICS($where: ArenaAnalyticsWhereInput) {
  ...
  try {
    ...
    if (arenaAnalytics) {
      ...
      arenaAnalytics.forEach(item => {
        spotData.push({
          date: item.date,
          total: item.tcCreatedCount.spot,
          cumulativeTotal: item.tcCreatedCount.cumulativeSpot,
        })
        perpetualData.push({
          date: item.date,
          total: item.tcCreatedCount.perpetual,
          cumulativeTotal: item.tcCreatedCount.cumulativePerpetual,
        })
        totalData.push({
          date: item.date,
          total: item.tcCreatedCount.total,
          cumulativeTotal: item.tcCreatedCount.cumulativeTotal,
        })
      })
