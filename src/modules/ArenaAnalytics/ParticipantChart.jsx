...
const fetchCreatedTC = async period => {
  try {
    ...
    const { arenaAnalytics } = await ArenaClient.request(V4_PARTICIPANT_ANALYTICS, {
      where,
    })
    if (arenaAnalytics) {
      const spotData = []
      ...
      arenaAnalytics.forEach(item => {
        spotData.push({
          date: item.date,
          total: item.participantsCount.spot,
          cumulativeTotal: item.participantsCount.cumulativeSpot,
        })
        perpetualData.push({
          date: item.date,
          total: item.participantsCount.perpetual,
          cumulativeTotal: item.participantsCount.cumulativePerpetual,
        })
        totalData.push({
          date: item.date,
          total: item.participantsCount.total,
          cumulativeTotal: item.participantsCount.cumulativeTotal,
        })
      })
